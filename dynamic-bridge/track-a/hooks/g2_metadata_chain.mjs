...
function init(domainName) {
  ...
  try {
    ...
    reportLargeAnonymousRw();
    report({
      event: 'G2_CHAIN_SUMMARY',
      installed: g2Installed,
      base: text(g2Base),
      ctr_calls: g2CtrCalls.length,
      prf_enters: g2PrfEnters.count,
      lazy_hits: Object.fromEntries(g2LazyHits),
      name_getter_derived: 'see G2_CHAIN name-table-lazy-getter head16',
    });
    report({ event: 'A2_INIT_RETURN', selector: 'il2cpp_init', domain_name: name, domain_ptr: domainPtr });
    return {
      selector: 'il2cpp_init',
      status: 'returned',
      domain_ptr: domainPtr,
      g2: {
        installed: g2Installed,
        base: text(g2Base),
        ctr_calls: g2CtrCalls.length,
...
// ==== G2 metadata chain anchors (T1) appended to the accepted A2 surface ====
function safeString(pointer) {
  ...
  try {
    ...
    return pointer.readUtf8String(128);
  } catch (_) {
    return null;
  }
}
...
const G2_ANCHORS = [
  { rva: '0x403F234', name: 'il2cpp_set_data_dir', kind: 'entry', tag: 'SET_DATA_DIR' },
  { rva: '0x3FE600C', name: 'il2cpp_init', kind: 'entry', tag: 'INIT' },
  { rva: '0x4025598', name: 'Runtime::Init', kind: 'entry', tag: 'RUNTIME_INIT' },
  { rva: '0x3FDD77C', name: 'MetadataCache::Initialize', kind: 'entry', tag: 'METADATA_INIT' },
  { rva: '0x4004540', name: 'CTR-decryptor', kind: 'ctr', tag: 'CTR' },
  { rva: '0x4004760', name: 'keystream-PRF', kind: 'count-only', tag: 'PRF' },
  { rva: '0x4025314', name: 'metadata-file-reader', kind: 'reader', tag: 'FILE_READER' },
  { rva: '0x4004CE0', name: 'buffer-processor', kind: 'buffer', tag: 'BUFFER' },
  { rva: '0x3B95A08', name: 'name-table-lazy-getter', kind: 'lazy', tag: 'NAME_GETTER' },
  { rva: '0x3FE09CC', name: 'lazy-getter', kind: 'lazy', tag: 'LAZY' },
  { rva: '0x3FE1914', name: 'lazy-getter', kind: 'lazy', tag: 'LAZY' },
  { rva: '0x3FE0AD8', name: 'lazy-getter', kind: 'lazy', tag: 'LAZY' },
  { rva: '0x3FE2338', name: 'lazy-getter', kind: 'lazy', tag: 'LAZY' },
  { rva: '0x3FE1A20', name: 'lazy-getter', kind: 'lazy', tag: 'LAZY' },
  { rva: '0x3FE2960', name: 'lazy-getter', kind: 'lazy', tag: 'LAZY' },
  { rva: '0x3FE2E40', name: 'lazy-getter', kind: 'lazy', tag: 'LAZY' },
  { rva: '0x3FE2F4C', name: 'lazy-getter', kind: 'lazy', tag: 'LAZY' },
  { rva: '0x3FE3E54', name: 'lazy-getter', kind: 'lazy', tag: 'LAZY' },
];
let g2Seq = 0;
let g2Installed = false;
let g2Base = null;
const g2CtrCalls = [];
const g2PrfEnters = { count: 0 };
const g2LazyHits = new Map();
const g2AnchorStates = new Map();
const G2_LAZY_RECORD_CAP = 64; // cap JSONL rows for high-frequency lazy/name getters; keep counting silently

function g2AddrRva(address) {
  if (address === null || address === undefined || g2Base === null) {
    return null;
  }
  try {
    return address.sub(g2Base).toString();
  } catch (_) {
    return null;
  }
}

function g2HeadHex(pointer, nbytes) {
  if (pointer === null || pointer === undefined) {
    return null;
  }
  try {
    ...
    const bytes = pointer.readByteArray(nbytes);
    return Array.prototype.map.call(
      new Uint8Array(bytes),
      (byte) => ('0' + byte.toString(16)).slice(-2),
    ).join('');
...
function g2Emit(anchor, phase, args, ret, extra) {
  g2Seq += 1;
  const event = {
    event: 'G2_CHAIN',
    ts: Date.now(),
...
function g2BacktraceRvas(context) {
  try {
    if (typeof Thread === 'undefined' || typeof Thread.backtrace !== 'function') {
      return [];
    }
    return Thread.backtrace(context, Backtracer.ACCURATE).slice(0, 24)
      .map((frame) => {
        if (frame === null || frame === undefined || typeof frame.sub !== 'function' || g2Base === null) {
          return null;
        }
        try {
          return frame.sub(g2Base).toString();
        } catch (_) {
          return null;
        }
      })
...
function g2InstallAnchor(anchor, address) {
  const onEnter = function (args) {
    const ctx = this;
    if (anchor.kind === 'count-only') {
      g2PrfEnters.count += 1;
      g2Emit(anchor, 'enter', null, null, { prf_enter_count: g2PrfEnters.count });
      return;
    }
    if (anchor.kind === 'ctr') {
      g2CtrCalls.push(Date.now());
      const keyPtr = args[0];
      const outPtr = args[2];
      g2Emit(anchor, 'enter', {
        key: text(keyPtr),
        key_rva: g2AddrRva(keyPtr),
        key_head16: g2HeadHex(keyPtr, 16),
        len: args[1] === undefined ? null : text(args[1]),
        out: text(outPtr),
        out_rva: g2AddrRva(outPtr),
      }, null, { ctr_call_index: g2CtrCalls.length });
      return;
    }
    if (anchor.kind === 'reader') {
      g2Emit(anchor, 'enter', {
        arg0_str: safeString(args[0]),
        arg1_str: safeString(args[1]),
        arg0: text(args[0]),
        arg1: text(args[1]),
      }, null, null);
      return;
    }
    if (anchor.kind === 'buffer') {
      g2Emit(anchor, 'enter', {
        arg0: text(args[0]),
        arg1: text(args[1]),
        arg2: text(args[2]),
    ...
    if (anchor.kind === 'lazy') {
      const key = anchor.rva;
      g2LazyHits.set(key, (g2LazyHits.get(key) || 0) + 1);
      if (g2LazyHits.get(key) <= G2_LAZY_RECORD_CAP) {
        const callers = g2BacktraceRvas(ctx);
        g2Emit(anchor, 'enter', {
          x0: text(args[0]),
          x1: text(args[1]),
          callers: callers.slice(0, 3),
          hit_count: g2LazyHits.get(key),
        }, null, null);
      }
      return;
    }
    if (anchor.tag === 'SET_DATA_DIR') {
      g2Emit(anchor, 'enter', { data_dir: safeString(args[0]), arg0: text(args[0]) }, null, null);
      return;
    }
    if (anchor.tag === 'INIT') {
      g2Emit(anchor, 'enter', { domain_name: safeString(args[0]), arg0: text(args[0]) }, null, null);
      return;
    }
    g2Emit(anchor, 'enter', {
      x0: text(args[0]),
  ...
  const onLeave = function (retval) {
    if (anchor.kind === 'count-only') {
      return;
    }
    const ret = text(retval);
    if (anchor.kind === 'ctr') {
      g2Emit(anchor, 'leave', null, { ret }, { ctr_call_index: g2CtrCalls.length });
      return;
    }
    if (anchor.kind === 'lazy' || anchor.tag === 'NAME_GETTER') {
      const hits = g2LazyHits.get(anchor.rva) || 0;
      if (hits <= G2_LAZY_RECORD_CAP) {
        const head = g2HeadHex(retval, 16);
        let retStr = null;
        if (anchor.tag === 'NAME_GETTER') {
          retStr = safeString(retval);
        }
        const out = { ret, head16: head };
        if (retStr !== null) {
          out.str = retStr;
        }
        g2Emit(anchor, 'leave', null, out, { hit_count: anchor.kind === 'lazy' ? g2LazyHits.get(anchor.rva) : undefined });
      }
      return;
    }
    g2Emit(anchor, 'leave', null, { ret }, null);
  };
  try {
    Interceptor.attach(address, { onEnter, onLeave });
    g2AnchorStates.set(anchor.rva, 'attached');
    report({ event: 'G2_CHAIN_ANCHOR_ATTACHED', rva: anchor.rva, name: anchor.name });
  } catch (error) {
    g2AnchorStates.set(anchor.rva, String(error));
    report({ event: 'G2_CHAIN_ANCHOR_ERROR', rva: anchor.rva, name: anchor.name, error: String(error) });
  }
}

function installG2Anchors() {
  if (g2Installed) {
    return;
  }
  const module = Process.findModuleByName(IL2CPP_LIBRARY);
  if (module === null || module === undefined) {
    report({ event: 'G2_CHAIN_INSTALL', status: 'error', error: 'libil2cpp.so not loaded' });
    return;
  }
  g2Base = module.base;
  for (const anchor of G2_ANCHORS) {
    report({ event: 'G2_CHAIN_ANCHOR_ATTACHING', rva: anchor.rva, name: anchor.name });
    g2InstallAnchor(anchor, g2Base.add(anchor.rva));
  }
  g2Installed = true;
  report({
    event: 'G2_CHAIN_INSTALL',
    status: 'installed',
    library: IL2CPP_LIBRARY,
    base: text(g2Base),
    module_size: module.size,
    anchors: G2_ANCHORS.map((a) => a.rva),
  });
}
...
function g2Status() {
  return {
    installed: g2Installed,
    base: text(g2Base),
    anchors: Array.from(g2AnchorStates.entries()).map(([rva, state]) => ({ rva, state })),
    ctr_calls: g2CtrCalls.length,
    prf_enters: g2PrfEnters.count,
    lazy_hits: Object.fromEntries(g2LazyHits),
  };
