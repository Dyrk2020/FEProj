#!/usr/bin/env python3
"""Fail-closed evidence builder for the G1 resource→loader timing edge.

Verifies two g1res run directories against the locked inputs and emits
track-b/g1-resource-loader-evidence.json.  Any check failure raises and writes
...
EVIDENCE_OUT = TRACK_B / "g1-resource-loader-evidence.json"
...
def verify_run(run_dir: Path, index: int) -> dict[str, Any]:
    ...
    # 4) ordered events: open -> read -> lua_load(feproj internal) -> luaL_loadbufferx
    ...
    # 7) caller RVA for the FEProj-internal lua_load is inside FEProj
    ...
    return {
        ...
        "direct_return_code": direct_event.get("return_code"),
...
def main(run_dirs: list[str]) -> int:
    ...
    evidence = {
        "verdict": "G1_RESOURCE_LOADER_EDGE_OBSERVED",
        "scope": "named resource -> FEProj loader (luaL_loadbufferx internal lua_load) timing edge only; "
                 "dispatch/execution chain NOT observed; G1 full verdict remains NOT_ESTABLISHED",
        "evidence_tier": "H",
