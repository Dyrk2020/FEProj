...
const hookPath = new URL("./hooks/g1_pcall_observe.js", import.meta.url);
...
const feprojModule = {
  name: "libFEProj.so",
  base: { toString: () => "0x70000000", add: () => ({ toString: () => "0x70010000", compare: () => 0 }), sub: () => ({ toString: () => "0x1000" }) },
  size: 148742144,
  findExportByName(name) {
    return ["luaL_newstate", "luaL_loadbufferx", "lua_load", "lua_pcallk"].includes(name)
      ? { name, toString: () => name }
      : null;
  },
};
...
assert.deepEqual(selectors, ["luaL_newstate", "luaL_loadbufferx", "lua_load", "lua_pcallk"]);
assert.ok(probes.every((entry) => entry.resolved));

const result = surface.runpcall();
assert.equal(result.load_return, 0);
assert.equal(result.pcall_return, 0);
assert.ok(sent.some((event) => event.event === "G1_PCALL_RESULT" && event.return_code === 0));
assert.ok(sent.some((event) => event.event === "LUA_PCALL" && event.return_code === 0 && event.nargs === 0 && event.nresults === 0));
