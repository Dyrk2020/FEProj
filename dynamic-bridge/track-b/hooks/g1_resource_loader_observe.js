...
// G1 resource→loader observation hook (observe-only, built on B2-R1).
//
// Records the true timing edge:
//   FETEST_OPEN -> FETEST_READ (locked BattleLogic.res bytes) -> LUA_LOAD.
//
// Provenance binding: the buffer passed into luaL_loadbufferx is the exact
// staged file bytes (read through the observed open/read surface), and the
// event carries the full buffer hex so the host can SHA-256 it against the
// locked resource.  A controlled RPC `runresource` (form (b) in the plan)
// performs luaL_newstate -> read staged file -> luaL_loadbufferx.
...
const MAX_HEX_BYTES = 4096; // full 1479-byte resource is comfortably below this
...
function readBytesHex(buffer, maxBytes) {
  if (buffer === null || buffer === undefined) {
    return null;
  }
  try {
    let count = maxBytes;
    if (typeof buffer.readByteArray === "function") {
      // readByteArray(size) is the only supported form here; size is capped.
    }
...
function headHex(buffer, byteCount) {
  return readBytesHex(buffer, byteCount);
}
...
function runResource() {
  // Form (b): controlled call. luaL_newstate -> staged locked bytes ->
  // luaL_loadbufferx with the real staged chunk name. Provenance is bound by
  // the open/read events plus buffer SHA in the LUA_LOAD event.
  if (!feprojLoaded || feprojModule === null) {
    throw new Error("libFEProj.so is not loaded");
  }
  const newstate = moduleLuaExport(feprojModule, "luaL_newstate");
  const loadbufferx = moduleLuaExport(feprojModule, "luaL_loadbufferx");
  if (newstate === null || loadbufferx === null) {
  ...
  const returnCode = numberValue(load(
    state,
    buffer,
    1479,
  ...
  send({
    event: "G1_RESOURCE_LOAD",
    chunk_name: chunkName,
    state: pointerKey(state),
    return_code: returnCode,
  });
  return { state: pointerKey(state), return_code: returnCode, chunk_name: chunkName };
}
